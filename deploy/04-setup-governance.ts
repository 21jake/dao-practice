...
const setupGovenor: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
  ...
  log(
    `Succesfully set up governor contract as proposer in timelock contract: ${receiptProposerTx.transactionHash}`
  )
  ...
  log(
    `Succesfully set up address zero as executor in timelock contract: ${receiptExecutorTx.transactionHash}`
  )

  const revokeTx = await timeLock.revokeRole(adminRole, deployer)
  const revokeReceipt = await revokeTx.wait(1)
  log(`Succesfully voided the admin role: ${revokeReceipt.transactionHash}`)
